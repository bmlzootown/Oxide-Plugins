...
namespace Oxide.Plugins {
	...
	class CommandBook : RustPlugin {
        ...
        void LoadVariables() {
            dropMinX = Convert.ToSingle(-((World.Size/2) - 500));
            dropMaxX = Convert.ToSingle((World.Size / 2) - 500);
            dropMinZ = Convert.ToSingle(-((World.Size / 2) - 500));
            dropMaxZ = Convert.ToSingle((World.Size / 2) - 500);
            dropMinY = Convert.ToSingle(200f);
            dropMaxY = Convert.ToSingle(300f);
            dropMinCrates = Convert.ToInt32(1);
            dropMaxCrates = Convert.ToInt32(3);
            minDropCratesInterval = Convert.ToInt32(3);
            maxDropCratesInterval = Convert.ToInt32(10);
            showDropLocation = Convert.ToBoolean(false);
            airdropSpeed = Convert.ToSingle(40f);
        }
	   ...
	   void OnEntitySpawned(BaseEntity entity) {
		...
		[ChatCommand("cdrop")]
		void cmdCDrop(BasePlayer player, string command, string[] args) {
            if (player.net.connection.authLevel < 1) {
                    SendReply(player, "Error: You do not have permission to use this command.");
                    return;
            }
            if (args == null || args.Length < 3) {
                SendReply(player, "Format: /cdrop [x] [y] [z]");
                return;
            }
            AllowNextDrop(); 
            ...
            if (entity != null)
            {
                ...
                targetPos.x = Convert.ToSingle(args[0]);
                targetPos.y = Convert.ToSingle(args[1]);
                targetPos.z = Convert.ToSingle(args[2]);
                dropPoint.Add(entity as CargoPlane, targetPos);
                entity.Spawn(true);
                dropPoint.Remove(entity as CargoPlane);
				SendReply(player, "Dropping package at X=" + args[0] + ", Y=" + args[1] + ", Z=" + args[2]);
            }
        }
		
		[ChatCommand("pdrop")]
		void cmdChatPDrop(BasePlayer player, string command, string[] args) {
            ...
            if (args == null || args.Length < 1) {
                SendReply(player, "Format: /pdrop [player]");
                return;
            }
			...
			if (playz.Contains(SendTo)) {
				...
				if (entity != null) {
					var targetPos = new Vector3();
					targetPos.x = Convert.ToSingle(SendzTo.transform.position.x + 2);
					targetPos.y = Convert.ToSingle((SendzTo.transform.position.y + 30));
					targetPos.z = Convert.ToSingle(SendzTo.transform.position.z + 2);
					dropPoint.Add(entity as CargoPlane, targetPos);
					entity.Spawn(true);
					dropPoint.Remove(entity as CargoPlane);
					SendReply(player, "Dropping package near " + SendTo);
					SendReply(SendzTo, "What's that I hear in the distance? Could it be... a plane?");
				}
			} else {
					SendReply(player, "Player is not currently online.");
			}
		...
		[ChatCommand("coords")]
		void cmdChatCoords(BasePlayer player, string command, string[] args) {
			SendReply(player, "X=" + Convert.ToString(player.transform.position.x) + ", Y="  + Convert.ToString(player.transform.position.x) + ", Z=" + Convert.ToString(player.transform.position.z));
		}
	}
}